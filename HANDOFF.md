...
### 3-0. 문의 폼 — 브라우저 직접 전송으로 복구함 (2026-08-11)

한동안 폼이 502 를 돌려주고 **문의가 한 건도 전달되지 않았습니다.** 이관하며 전달을 서버 경유로 바꿨는데, Web3Forms 무료 플랜이 서버 호출을 막기 때문이었습니다.
...
무료 플랜에서 동작하는 유일한 경로가 브라우저 직접 호출이라, `ContactForm.tsx` 의 `DELIVERY_MODE` 를 `'web3forms'` 로 두어 복구했습니다.

**그래서 `NEXT_PUBLIC_WEB3FORMS_ACCESS_KEY` 는 지금 필요합니다. 지우지 마세요.** (앞서 이 노트는 이 변수를 지우라고 안내했는데, 서버 경유를 전제한 내용이라 더는 맞지 않습니다.)

**남은 개선 — 키를 번들에서 없애기.** 이 키는 `NEXT_PUBLIC_` 이라 빌드 결과에 그대로 실립니다. Web3Forms 도 공개 값으로 전제하고 자체 스팸 필터를 걸고, 폼에도 캡차·허니팟이 있습니다. 그래도 키를 아는 누구나 이 폼으로 메일을 보낼 수 있다는 점은 남습니다. 없애려면 둘 중 하나입니다.

| 방법 | 비용 | 필요한 작업 |
|---|---|---|
| **Resend 연동** (권장) | 무료 3,000통/월 | Cloudflare Secret 에 `RESEND_API_KEY`·`CONTACT_TO_EMAIL` 추가 → `DELIVERY_MODE` 를 `'server'` 로. `dmns.co.kr` 에 SPF·DKIM 필요(자체 DNS라 바로 가능) |
| **Web3Forms Pro** | 유료 | 결제 후 `DELIVERY_MODE` 를 `'server'` 로 |

전환은 상수 한 줄이라, 키만 준비되면 바로 됩니다.
...
화면에 표시되는 이메일은 `dmnsolution251@gmail.com` 으로 바꿨지만, **실제 접수 메일이 가는 주소는 Web3Forms 계정에 등록된 주소**를 따릅니다. 대시보드에서 확인하세요.

> Cloudflare 환경변수 `NEXT_PUBLIC_WEB3FORMS_ACCESS_KEY` 는 **그대로 두어야 합니다** — 3-0 참고. 서버 전용 `WEB3FORMS_ACCESS_KEY` 시크릿도 남아 있는데, `DELIVERY_MODE` 를 `'server'` 로 되돌릴 때 쓰이므로 지우지 않아도 됩니다(무료 플랜에서는 그 경로가 403 이라 실제로는 쓰이지 않습니다).
